...
class coap:
	...
	def QUADRO(self, recurso):
		...
		if(self.metodo == separado):
			lista_recurso = recurso.split('/')
			for uri_path in lista_recurso:
				self.optionsdelta = DELTA.URI_PATH.value[0] - self.optionsdelta
				self.optionslength = len(uri_path)
				quadro += (self.optionsdelta << 4 | self.optionslength).to_bytes(1, byteorder='big')
	...
	def GET(self, recurso, server_adress, port):
		self.tipo = T.CONFIRMABLE.value
		self.code = CODE_0.GET.value
		self.payload = b''
		self.caminho = server_adress
		self.porta = port
		self.metodo = 'separado'
		self.QUADRO(recurso)
		
		self.sock.sendto(self.quadro, (self.caminho, self.porta))
	...
	def POST(self, recurso, server_adress, port, entrada):
		self.tipo = T.CONFIRMABLE.value
		self.code = CODE_0.POST.value
		self.payload = entrada.encode()
		self.caminho = server_adress
		self.porta = port
		self.metodo = 'separado'
		self.QUADRO(recurso)
	...
	def PUT(self, recurso, server_adress, port, entrada):
		self.tipo = T.CONFIRMABLE.value
	...
	def DELETE(self, recurso, server_adress, port)
		self.tipo = T.CONFIRMABLE.value
		self.code = CODE_0.GET.value
		self.payload = b''
		self.caminho = server_adress
